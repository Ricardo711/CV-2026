...
import type { FourClass, Game2PredictResult, RoundData2, SubmitResult } from "../types/game";
...
export function useGame2() {
  ...
  async function predictAndAdvance(sessionId: string, round: number): Promise<void> {
    if (!selectedFile.value || !firstAnswer.value || firstConfidence.value == null) return;
    if (!answerId.value) return;

    const elapsed = step1StartTime != null ? (Date.now() - step1StartTime) / 1000 : 0;
  ...
  async function finalize(): Promise<SubmitResult | null> {
    if (!aiResult.value || !finalAnswer.value || trustInAi.value == null || aiConfidenceRating.value == null) return null;
