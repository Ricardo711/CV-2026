...
export async function getRoundData(sessionId: string, round: number): Promise<RoundData> {
...
export async function predictGame2(formData: FormData): Promise<Game2PredictResult> {
...
export async function finalizeGame2(
  ...
  return api<SubmitResult>(`/api/games/answers/${encodeURIComponent(answerId)}/finalize`, {
    method: "PATCH",
    body: params,
  });
}
